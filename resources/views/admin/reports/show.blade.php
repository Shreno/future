...
<body>
    <div style="">
        <caption>فاتورة رقم {{$Invoices_details->InvoceNum}} <br>
        <a 
                            href="#!" onclick="window.print();">طباعة</a>     
                            <a 
                            href="{{url('admin/reportpdf/'.$Invoices_details->id.'')}}" >تحميل</a>    </caption>
    </div>
    ...
    <table style="width: 100%;">
        ...
        <tr>
            ...
            <td>
                ...
                <address style="color: #52526C;opacity: 0.8; margin-top: 10px; font-style:normal;">
                    <span style="font-size: 18px; line-height: 1.5; font-weight: 500;">
                        رقم الفاتورة :{{$Invoices_details->InvoceNum}}</span>
                </address>  <address style="color: #52526C;opacity: 0.8; margin-top: 10px; font-style:normal;">
                    <span style="font-size: 18px; line-height: 1.5; font-weight: 500;">
                         تاريخ الفاتورة :{{$Invoices_details->created_at}}</span>
                </address>
                <address style="color: #52526C;opacity: 0.8; margin-top: 10px; font-style:normal;">
                    <span style="font-size: 18px; line-height: 1.5; font-weight: 500;">
    ...
    <table style="width: 100%;">
        ...
        @php  $total=0;$Glopaltotal=0;$tax=0; $shippingCost=0; $allreturnCost=0; $allcostfess=0;  @endphp
        ...
        <tr>
            ...
            <td>{{$order->amount}}</td>
            <td>
            @php $costfees =$order->fees_cash_on_delivery; 
            $allcostfess+=$costfees;
            ...
            <td>
                ...
                @else
                  ...
                  @php $costReturn = $order->user->cost_reshipping; 
                  $allreturnCost+=$costReturn;
                  ...
                  @endphp
                    ...
                    {{$costReturn}} 
                    @else
                    @php $costReturn = $order->user->cost_reshipping_out_city;
                    $allreturnCost+=$costReturn;
            ...
            @if($user->work==2)
            @php $cost = $order->user->cost_inside_city; 
            $shippingCost+=$cost;
            ...
            @elseif($order->sender_city != $order->receved_city)

            @php $cost = $order->user->cost_outside_city; 
            $shippingCost+=$cost;
            @endphp
            {{$cost}}
            @endif
            ...
            <td>
            {{$cost * $order->user->tax / 100 +$total}}
            </td>
            
            @php
            $tax+=$cost * $order->user->tax / 100; 
            $Glopaltotal +=$tax + $total;
