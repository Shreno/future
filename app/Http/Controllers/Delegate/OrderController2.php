...
class OrderController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($order) {
            ...
            {
                    if ($order->status_id = $order->user->cost_calc_status_id && $order->amount_paid==1) 
                    {                 
                        ...
                        if (!$transaction) 
                        {
                                ...
                                //     "trans_type" => 'cost',
                                // ]);
                                if(auth()->user()->work_type=3)
                                {
                                  ClientTransactions::create([
                                    "user_id" => auth()->user()->id,
                                    "description" => 'شحن طلب ',
                                    "creditor" => auth()->user()->payment,
                                    "order_id" => $order->id,
                                    "trans_type" => 'cost',
                                  ]);
                                }
                    ...
                    elseif ($order->status_id = $order->user->cost_calc_status_id && $order->amount_paid==0) 
                    {
