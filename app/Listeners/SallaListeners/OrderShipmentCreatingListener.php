...
class OrderShipmentCreatingListener
{
     ...
     * Handle the event.
     *
     * @param  OrderShipmentCreating  $event
     * @return void
     */
    ...
    private function createOrder($webHookPayloadObject)
    {
        ...
        if(! isset($webHookPayloadObject->data->items)){ return; }
        ...
        $referenceNumber = $webHookPayloadObject->data->reference_id;

        $providerOrder = ProviderOrder::where('provider_order_id',$webHookPayloadObject->data->id)->first();
    ...
    private function saveStoreAddress($userID,$storeCityId,$shippingData)
    {
        ...
        if(!$storeAddress) {
            Address::create([ 'user_id' => $userID, 'city_id' => $storeCityId, 'address' =>
                $shippingData->pickup_address->shipping_address, 'description' => $shippingData->pickup_address->street_number .' '.
